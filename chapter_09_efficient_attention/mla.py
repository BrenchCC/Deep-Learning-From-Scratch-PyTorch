...
class MultiHeadLatentAttention(nn.Module):
    """
    Educational low-rank latent attention.

    This module compresses K/V into a low-rank latent space and reconstructs
    them back to head space before attention.
    """
    ...
    def __init__(self, config: AttentionConfig):
        ...
        self.key_compress_projection = nn.Linear(self.d_model, self.latent_dim)
        self.value_compress_projection = nn.Linear(self.d_model, self.latent_dim)

        self.key_expand_projection = nn.Linear(self.latent_dim, self.d_model)
        self.value_expand_projection = nn.Linear(self.latent_dim, self.d_model)
    ...
    def forward(
        ...
        key_latent = self.key_compress_projection(hidden_states)
        value_latent = self.value_compress_projection(hidden_states)

        key = self._split_heads(self.key_expand_projection(key_latent))
        value = self._split_heads(self.value_expand_projection(value_latent))
