...
## 3分钟先读（零基础导读）
- 本章核心结论：只有线性层不够，必须加入非线性激活函数，MLP 才能拟合复杂函数。
- 你会看到两个任务：`1D 正弦拟合` 与 `2D 曲面拟合`，对应同一份 MLP 代码的两条分支。
- 学习建议：先看结果图，再反看网络结构和训练循环。

## 术语速查（本章高频）
| 术语 | 一句话解释 |
|---|---|
| MLP | 由全连接层堆叠而成的前馈神经网络 |
| 激活函数 | 给网络引入非线性能力的关键算子 |
| 万能逼近定理 | 说明单隐层网络在条件满足时可逼近任意连续函数 |
| 欠拟合 | 模型太简单，训练集都拟合不好 |
| 过拟合 | 模型记住训练数据但泛化差 |
| 表示学习 | 网络每层都在学习更有用的特征表达 |

## 理论 -> 代码映射表
| 理论主题 | 对应代码位置 | 你会看到什么 |
|---|---|---|
| MLP 结构 | `class MLP` in `mlp.py` | 线性层 + ReLU + 线性层 |
| 1D 非线性拟合 | `generate_sine_data(...)` + `visualize_results(...)` | 正弦曲线拟合结果 |
| 2D 曲面拟合 | `generate_surface_data(...)` + `visualize_3d_results(...)` | `z = sin(x) + cos(y)` 曲面拟合 |
| 训练主循环 | `train_model(...)` in `mlp.py` | forward -> zero_grad -> backward -> step |
| 命令行入口 | `parse_args()` + `mlp_demo()` | 通过参数控制 1D/2D 与训练配置 |

## 常见误区 + 最小运行命令
### 常见误区
- 误区 1：层数越深一定越好。  
  正解：任务规模、数据量、优化难度都要匹配。
- 误区 2：看到训练 loss 降低就代表泛化很好。  
  正解：要结合测试区间或外推区间观察。
- 误区 3：2D 可视化只是“好看”。  
  正解：它能直观看到 MLP 是否学到了目标函数形状。

### 最小运行命令
```bash
# 1D 正弦拟合
python chapter_02_nn_basics_mlp/mlp.py --input_dim 1 --mode standard

# 2D 曲面拟合
python chapter_02_nn_basics_mlp/mlp.py --input_dim 2
```
